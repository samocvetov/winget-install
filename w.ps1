# Принудительно переключаем консоль в режим UTF-8
...
# --- Блок обновления ---
Write-Host "`n--- Проверка обновлений ---" -ForegroundColor Cyan
# Получаем список обновлений
$updateList = winget upgrade | Select-String -Pattern '^\S+' | Select-Object -Skip 2
...
foreach ($line in $updateList) {
    ...
    if ($id -and $id -ne "ID") {
        $confirm = Read-Host "Обновить $name ($id)? [y/n]"
        if ($confirm -eq 'y') {
...
# --- Блок установки ---
Write-Host "`n--- Установка пакетов ---" -ForegroundColor Cyan
foreach ($app in $appsToInstall) {
    ...
    if ($confirmation -eq 'y') {
        ...
        if ($process.ExitCode -eq 0) {
            ...
            Write-Host "`r[FAIL] $app (Ошибка: $($process.ExitCode))" -ForegroundColor Red
        }
    }
}
